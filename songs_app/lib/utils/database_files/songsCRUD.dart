...
class SongsCRUD {
  ...
  Future<List<Map<String,dynamic>>> getSongMapList() async {
    ...
    List<Map<String,dynamic>> result = await db.rawQuery('SELECT * FROM ${SongsTable.tableName}');
    return result;
  }
  ...
  Future<int> updateSong(Song song) async {
    ...
    int result = await db.update(SongsTable.tableName, song.toMap(), where: '${SongsTable.colSongId} : ?', whereArgs: [song.songId]);
  ...
  Future<int> deleteSong(String songTitle) async {
    ...
    int result = await db.rawDelete('DELETE FROM ${SongsTable.tableName} WHERE ${SongsTable.colTitle} = \'$songTitle\'');
    return result;
  }  
  ...
  Future<int> getTotalSongCount() async {
    ...
    List<Map<String,dynamic>> countMap = await db.rawQuery('SELECT COUNT (*) FROM ${SongsTable.tableName}');
  ...
  Future<List<Map<String,dynamic>>> getSongMapById(String title) async {
    ...
    List<Map<String,dynamic>> result = await db.rawQuery('SELECT * FROM ${SongsTable.tableName} WHERE ${SongsTable.colTitle} = $title');
    return result;
  }
