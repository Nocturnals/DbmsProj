...
class SongByCRUD {
  ...
  Future<List<Map<String,dynamic>>> getSongByMapList() async {
    ...
    List<Map<String,dynamic>> result = await db.rawQuery('SELECT * FROM ${SongByTable.tableName}');
    return result;
  }
  ...
  Future<int> updateSongBy(SongBy songby) async {
    ...
    int result = await db.update(SongByTable.tableName, songby.toMap(), where: '${SongByTable.colSongId} : ?', whereArgs: [songby.songId]);
  ...
  Future<int> deleteSongBy(int artistId) async {
    ...
    int result = await db.rawDelete('DELETE FROM ${SongByTable.tableName} WHERE ${SongByTable.colArtistId} = \'$artistId\'');
    return result;
  }  
  ...
  Future<int> getTotalSongByCount() async {
    ...
    List<Map<String,dynamic>> countMap = await db.rawQuery('SELECT COUNT (*) FROM ${SongByTable.tableName}');
  ...
  Future<List<Map<String,dynamic>>> getSongByMapById(int artistId) async {
    ...
    List<Map<String,dynamic>> result = await db.rawQuery('SELECT * FROM ${SongByTable.tableName} WHERE ${SongByTable.colArtistId} = $artistId');
    return result;
  }
}