...
class ArtistsCRUD {
  ...
  Future<List<Map<String,dynamic>>> getArtistMapList() async {
    ...
    List<Map<String,dynamic>> result = await db.rawQuery('SELECT * FROM ${ArtistTable.tableName}');
    return result;
  }
  ...
  Future<int> updateArtist(Artist artist) async {
    ...
    int result = await db.update(ArtistTable.tableName, artist.toMap(), where: '${ArtistTable.colArtistId} : ?', whereArgs: [artist.artistId]);
  ...
  Future<int> deleteArtist(String name) async {
    ...
    int result = await db.rawDelete('DELETE FROM ${ArtistTable.tableName} WHERE ${ArtistTable.colName} = \'$name\'');
    return result;
  }  
  ...
  Future<int> getTotalArtistCount() async {
    ...
    List<Map<String,dynamic>> countMap = await db.rawQuery('SELECT COUNT (*) FROM ${ArtistTable.tableName}');
  ...
  Future<List<Map<String,dynamic>>> getArtistMapById(String name) async {
    ...
    List<Map<String,dynamic>> result = await db.rawQuery('SELECT * FROM ${ArtistTable.tableName} WHERE ${ArtistTable.colName} = $name');
    return result;
  }
}